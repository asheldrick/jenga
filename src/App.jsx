...
const S = {
  ...
  header: { textAlign:"center", padding:"2rem 0 1.5rem", borderBottom:`2px solid ${COPPER_DIM}`, marginBottom:"1.5rem" },
  logo: { fontSize:"3rem", fontWeight:"bold", letterSpacing:"0.4em", color:COPPER, fontFamily:"'Georgia',serif" },
  tagline: { fontSize:"0.75rem", letterSpacing:"0.2em", color:INK_LIGHT, textTransform:"uppercase", marginTop:"0.25rem" },
  authBar: { display:"flex", alignItems:"center", justifyContent:"space-between", background:PANEL, border:`1px solid ${PARCHMENT_DARK}`, borderRadius:5, padding:"0.5rem 0.75rem", marginBottom:"0.5rem", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" },
  authInfo: { fontSize:"0.75rem", color:INK_LIGHT },
  authEmail: { color:INK_MID, fontWeight:"bold" },
  authBtn: { background:COPPER, color:PARCHMENT, border:"none", borderRadius:4, padding:"0.3rem 0.8rem", cursor:"pointer", fontSize:"0.75rem", fontFamily:"inherit", fontWeight:"bold" },
  authBtnOut: { background:"none", border:`1px solid ${PARCHMENT_DARK}`, color:INK_LIGHT, borderRadius:4, padding:"0.3rem 0.8rem", cursor:"pointer", fontSize:"0.75rem", fontFamily:"inherit" },
  syncMsg: { fontSize:"0.7rem", color:COPPER, fontStyle:"italic", marginLeft:"0.75rem" },
  dataBar: { display:"flex", alignItems:"center", gap:"0.4rem", background:PANEL, border:`1px solid ${PARCHMENT_DARK}`, borderRadius:5, padding:"0.5rem 0.75rem", marginBottom:"0.5rem", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" },
  dataBarLabel: { fontSize:"0.65rem", color:INK_LIGHT, letterSpacing:"0.15em", textTransform:"uppercase", marginRight:"0.25rem", flexShrink:0 },
  dataBtn: { background:PANEL2, border:`1px solid ${PARCHMENT_DARK}`, color:INK_MID, borderRadius:4, padding:"0.3rem 0.65rem", cursor:"pointer", fontSize:"0.72rem", fontFamily:"inherit", flexShrink:0 },
  ...
  stageBtn: { width:"100%", background:PANEL2, border:`1px solid ${PARCHMENT_DARK}`, color:INK_LIGHT, borderRadius:4, padding:"0.4rem 0.3rem", cursor:"pointer", fontSize:"0.7rem", display:"flex", flexDirection:"column", alignItems:"center", gap:"2px", transition:"all 0.15s" },
  ...
  overrideTick: { fontSize:"0.7rem", color:COPPER_DIM, userSelect:"none" },
  footerNote: { textAlign:"center", fontSize:"0.65rem", color:INK_LIGHT, marginTop:"1.5rem", lineHeight:1.8 },
  ...
  explainBox: { background:PANEL, border:`1px solid ${PARCHMENT_DARK}`, borderRadius:6, padding:"1.2rem 1.4rem", marginBottom:"1rem", boxShadow:"0 1px 4px rgba(0,0,0,0.07)" },
  ...
  startBtn: { width:"100%", background:COPPER, color:PARCHMENT, border:"none", borderRadius:5, padding:"0.85rem", fontSize:"1rem", fontWeight:"bold", cursor:"pointer", letterSpacing:"0.1em", fontFamily:"inherit" },
  ...
  choiceBtn: { background:PANEL, border:`1px solid ${PARCHMENT_DARK}`, color:INK_MID, borderRadius:5, padding:"0.8rem 0.6rem", cursor:"pointer", fontSize:"0.95rem", textAlign:"center", fontFamily:"inherit", transition:"all 0.15s", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" },
  ...
  resultsAdvice: { background:RED_BG, border:`1px solid ${RED_FG}33`, borderRadius:5, padding:"0.6rem 0.8rem", fontSize:"0.78rem", color:RED_FG, textAlign:"center", maxWidth:400 },
...
export default function App() {
  ...
  async function syncFromCloud(userId) {
    ...
    if (cloud && Object.keys(cloud).length > 0) {
      setProgress(local => ({ ...local, ...cloud }));
      saveProgress({ ...loadProgress(), ...cloud });
    }
    setSyncMsg("Synced ✓");
  ...
  function manualMarkStage(frameId, stageIdx) {
    setProgress(prev => {
      ...
      return { ...prev, [frameId]: { highestStage: Math.max(fp.highestStage, stageIdx), stageScores, passed: stageIdx === 3 || fp.passed } };
    });
  }
  ...
  function nextCard(correct) {
    ...
    if (activeStage === 3) {
      ...
      if (isLast) { markStageComplete(activeFrame.id, 3, newScore.correct/newScore.total); setSessionScore(newScore); setMissedCards(newMissed); setView("results"); }
      else { setCardIdx(c=>c+1); setFlipped(false); setSelected(null); setTypedAnswer(""); setFeedback(null); if (inputRef.current) setTimeout(()=>inputRef.current?.focus(),100); setSessionScore(newScore); setMissedCards(newMissed); }
      return;
    }

    const newQueue = correct ? queue : [...queue, currentCard];
    if (isLast && correct) { markStageComplete(activeFrame.id, activeStage, newScore.correct/newScore.total); setSessionScore(newScore); setMissedCards([]); setView("results"); }
    else {
      const next = cardIdx+1;
      if (!correct) { setQueue(newQueue); setRound(r=>r+1); }
      setCardIdx(next); setFlipped(false); setSelected(null); setTypedAnswer(""); setFeedback(null);
  ...
  if (authLoading) return (
    <div style={{...S.root, display:"flex", alignItems:"center", justifyContent:"center"}}>
      <div style={{color:COPPER, fontSize:"1rem", letterSpacing:"0.2em"}}>LOADING...</div>
    </div>
  );
    ...
    <div style={S.root}>
        ...
        {user ? (
          <>
            <div style={S.authInfo}>
              <span style={S.authEmail}>{user.email}</span>
              {syncMsg && <span style={S.syncMsg}>{syncMsg}</span>}
            </div>
            <button style={S.authBtnOut} onClick={signOut}>Sign out</button>
          </>
        ) : (
          <>
            <div style={S.authInfo}>Sign in to sync across devices automatically</div>
            <button style={S.authBtn} onClick={signInWithGoogle}>Sign in with Google</button>
          </>
        )}
      </div>
      ...
      {!user && (
        <div style={S.dataBar}>
          ...
          <button style={{...S.dataBtn,...(showExportBox?S.dataBtnActive:{})}} onClick={toggleExport}>{showExportBox?"▲ Hide":"⬇ Export"}</button>
          <button style={{...S.dataBtn,...(showImportBox?S.dataBtnActive:{})}} onClick={toggleImport}>{showImportBox?"▲ Hide":"⬆ Import"}</button>
          <button style={{...S.dataBtn,...S.dataBtnReset}} onClick={()=>{ if(window.confirm("Reset ALL progress?")){ setProgress({}); }}}>✕ Reset</button>
        </div>
      )}

      {user && (
        <div style={{...S.dataBar, justifyContent:"flex-end"}}>
          <button style={{...S.dataBtn,...S.dataBtnReset}} onClick={()=>{ if(window.confirm("Reset ALL progress?")){ setProgress({}); saveCloudProgress(user.id,{}); }}}>✕ Reset all progress</button>
        </div>
      )}

      {showExportBox && (
        <div style={S.ioBox}>
          <div style={S.ioBoxLabel}>Select all and copy. Save in Notes. Paste back via Import on another device.</div>
          <textarea readOnly style={S.ioTextarea} rows={5} value={JSON.stringify(progress,null,2)} onFocus={e=>e.target.select()} onClick={e=>e.target.select()}/>
        </div>
      )}
      ...
      {showImportBox && (
        <div style={S.ioBox}>
          ...
          <textarea style={S.ioTextarea} rows={5} value={importText} onChange={e=>setImportText(e.target.value)} placeholder="Paste exported JSON here..."/>
        ...
        {activeStage===0 && (
          <div style={S.drillBody}>
            <div style={S.cardWrap} onClick={()=>setFlipped(f=>!f)}>
              ...
              {flipped && <><div style={S.divider}/><div style={S.cardAnswerLabel}>SWAHILI</div><div style={S.cardAnswer}>{card.sw}</div></>}
            </div>
            {flipped && (
          ...
          <div style={S.drillBody}>
            ...
            <div style={S.choiceGrid}>
              {choices.map((c,ci)=>{
                ...
                return <button key={ci} disabled={selected!==null} style={btn} onClick={()=>{setSelected(c);setTimeout(()=>nextCard(c===card.sw),900);}}>{c}</button>;
              })}
            </div>
          </div>
        )}
        ...
        {(activeStage===2||activeStage===3) && (
          <div style={S.drillBody}>
            ...
            <div style={S.writtenArea}>
              <input ref={inputRef} autoComplete="off" autoCorrect="off" spellCheck="false" placeholder="Type your answer..."
      ...
      <div style={S.root}>
        <div style={S.resultsWrap}>
          ...
          <div style={{...S.resultsVerdict,color:passed?COPPER:RED_FG}}>
            {activeStage<3?"✓ 100% — STAGE COMPLETE":passed?"✓ TEST PASSED":"✗ TEST FAILED"}
          </div>
